...
7. **ONNX Runtime preflight:** The separation commands (`separate`, `upgrade_to_four_stem`, `re_separate`, `batch_separate`) make sure the ONNX Runtime is ready inside the background worker. When the runtime is missing or corrupt, the bootstrap worker downloads, verifies, probes, and stages it. The application process then loads ORT under a separate watchdog, completes the first activation, and continues with model bootstrap and separation. Query runtime state through the `get_runtime_bootstrap_status` IPC command and the `runtime-bootstrap-*` events. A Windows target may list more than one active runtime (a DirectML build and a CPU-only build). `resolve_runtime` picks the artifact for the preferred execution provider. When a DirectML load times out, the backend records the `directml_disabled_by_runtime_timeout` marker and resolves to the CPU-only runtime on the next attempt. The `runtime-bootstrap-*` event payload then carries a `cpu_fallback_notice` field (see ADR 0023).
