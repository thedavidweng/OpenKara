...
| `failure_phase`       | `Option<"download" \| "install" \| "probe" \| "activate">`                                                                                                                                                                                    | Bootstrap step where a `failed` state's error originated. `None` when the failure cannot be classified (including payloads from older backends); consumers fall back to download-failure copy. The serializer omits the field when the value is `None`. |
| `cpu_fallback_notice` | `Option<String>`                                                                                                                                                                                                                              | Set when the active runtime is the CPU-only Windows artifact and a DirectML load timeout was recorded (see ADR 0023). The serializer omits the field when the value is `None`.                                                                          |
...
`failure_phase` classifies where the failure happened: `download` (archive
fetch or digest mismatch during transfer), `install` (extraction, per-file
verification, or slot write), `probe` (the dynamic library did not load —
in the worker or under the application-process watchdog), or `activate`
(slot activation). A snapshot rebuilt from a persisted activation failure
reports `probe`, because a recorded failure always means an installed
runtime could not be verified or loaded. The field is omitted when the
failure cannot be classified; consumers treat that as a download failure,
which matches the pre-field behavior.
