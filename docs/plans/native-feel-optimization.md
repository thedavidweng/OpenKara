...
## Phase 6: 双击列表播放歌曲时的卡顿 + macOS 彩虹圈（高优先级）
...
- **远程歌曲**（`needs_remote_download`，行 114-146）：先调用
  `start_track_loading()` 立即返回一个 `state: "loading"` 的快照，真正的下载 +
  解码放到 `std::thread::spawn` 的后台线程里做。UI 立刻能显示 spinner，不会卡。
- **本地歌曲**（行 148-180）：**同步**执行
  `load_playback_source → load_song_audio → decode::decode_file`，
...
1. **列表未虚拟化**：`SongList.tsx:51` 直接 `displaySongs.map(...)` 渲染**全部**歌曲，
...
3. **每个列表项渲染做 O(N) 工作**：`SongListItem.tsx:73`
   ...
   行 85-100 又对所有 playlist 构建 `Map` 并逐个 `contextSongIds.filter`；
...
| P0     | **Phase 6.4 / 6.1** | 收窄 snapshot 订阅 + 本地歌曲先发 loading 快照（消除彩虹圈 + 卡死感） | 2-3 h    |
| P1     | **Phase 1**         | 移除所有 `cursor: pointer`                                            | 15 min   |
| P1     | **Phase 5**         | 替换 Settings 中剩余的 `window.prompt`                                | 1-2 h    |
| P1     | **Phase 6.2 / 6.3** | 曲库列表虚拟化 + 列表项 O(N) 计算移出                                 | 半天     |
