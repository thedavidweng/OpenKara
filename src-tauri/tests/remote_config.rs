use openkara_lib::config::{
    load_config, AppConfig, RegisteredLibrary, RemoteLibraryConnectionConfig, RemoteLibraryProvider,
};
...
fn remote_registered_library_serializes_as_discriminated_union() {
    ...
    let object = json
        .as_object()
        .expect("remote library should serialize as object");
