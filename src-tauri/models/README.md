...
- `scripts/setup.sh` may populate this directory for local development and
  tests with the latest `htdemucs` release asset (resolved at runtime from the
  upstream `latest.json` manifest). It also stages the matching ONNX Runtime shared library under
