...
- `scripts/setup.sh` may populate this directory for local development and
  tests with the `htdemucs` release asset pinned by the catalog snapshot
  (`src-tauri/catalog/release-manifest.json`, resolved via
  `scripts/resolve-model.mjs`). It also stages the matching ONNX Runtime shared library under
