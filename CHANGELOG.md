...
- Fix timed-lyrics scrolling and add Spotify-style follow control: attach the follow guard only after the viewport mounts (loading/empty early-returns previously left guard null so every rAF overwrote scrollTop and blocked manual scrolling); unlock follow only on real wheel/scroll movement — not touchstart/click — and suppress unlock until seek resume writes finish so line-click seek cannot leave follow permanently unlocked; idle re-follow plus an icon-only Follow button sharing the contextual-reveal enter/exit treatment of the other lyric controls; native scrollTop auto-follow; preserve line springs across React 19 ref churn.
