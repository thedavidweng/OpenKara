...
- Case-insensitive sidecar lyrics extension matching ([#63](https://github.com/thedavidweng/OpenKara/issues/63)): `read_sidecar_lyrics` only checked lowercase extensions (`.ttml`, `.lys`, `.lrc`). On case-sensitive filesystems (Linux), files like `song.LRC` or `song.TTML` were not found. Now scans the parent directory and matches the file stem with any-cased extension, with a fallback to probing common case variants if the directory can't be read.
