...
- Fix streaming multi-stem sample-rate conversion position drift ([#58](https://github.com/thedavidweng/OpenKara/issues/58)): `render_streaming_two_stem` and `render_streaming_four_stem` advanced `render_frame` by the pre-computed source-frame budget (`ceil()+1`) instead of the actual consumed frames (`round()`). The 1-2 frame per callback difference accumulated ~170 ms over a 3-minute song and could stall the ring buffer after ~8 minutes. Now uses the actual `src_frames_consumed` returned by `render_streaming_single`.
