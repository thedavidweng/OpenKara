...
- Propagate cache write errors in `import_lyrics_files` ([#61](https://github.com/thedavidweng/OpenKara/issues/61)): `upsert_lyrics_cache_entry` failures were silently discarded with `let _ =`, so files were reported as "matched" but lyrics were never stored. Now write failures move the file to `unmatched` and log the error.
