...
- Reuse HTTP clients across lyrics fetches for connection pool sharing ([#66](https://github.com/thedavidweng/OpenKara/issues/66)): `fetch_lyrics` and `fetch_lyrics_online` created new `LrcLibClient` and `LrcApiClient` (each with a fresh `reqwest::Client`) on every call, paying a full TLS handshake to LRCLIB/LrcAPI on every song switch. Now the clients are stored in `AppState` and cloned per call, sharing the internal connection pool.
