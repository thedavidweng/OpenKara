...
- Fix lyrics offset adjustment race condition on rapid clicks ([#62](https://github.com/thedavidweng/OpenKara/issues/62)): `adjustOffset` read `offsetMs` before the async `setLyricsOffset` call resolved, so two rapid clicks could both read the same stale value and only one delta was applied. Now uses an optimistic update (sets `offsetMs` synchronously before the await) with rollback on failure.
