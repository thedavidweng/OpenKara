...
- Keep lyrics auto-follow locked after line-click and seek-bar jumps in the packaged Tauri app: the player now publishes a host-owned seek revision only after the asynchronous seek response installs its authoritative target position, so AMLL `isSeek` / `resetScroll` cannot be consumed against the old playhead. Bare WKWebView layout-driven scroll events no longer masquerade as user scrolling; follow unlocks from wheel, touch movement, or an active pointer scrollbar drag.
