...
- Omit `duration=0` from LRCLIB lyrics lookup when song duration is unknown ([#57](https://github.com/thedavidweng/OpenKara/issues/57)): `lookup_query_from_song` now sets `duration_seconds` to `None` when `duration_ms == 0` instead of sending `duration=0`, which could cause LRCLIB to return no match or match the wrong song.
