...
- Add 7-day TTL to lyrics negative cache ([#59](https://github.com/thedavidweng/OpenKara/issues/59)): When online lyrics lookup returned no match, the result was cached as `LyricsSource::Absent` permanently — lyrics later added to LRCLIB/LrcAPI would never be discovered. Now Absent entries expire after 7 days, after which the next `fetch_lyrics` re-queries the network. Network errors were already not cached (only definitive no-match results are).
